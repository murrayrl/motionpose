...
def draw_trail(trail, color):
    """Helper to draw a trail on the screen."""
    if len(trail) < 2:
        return
...
def update_trail(trail, new_point):
    """Update a motion trail with a new point."""
...
def app_callback(pad, info, user_data):
    ...
    # Ensure HAILO_LANDMARKS is an enum, not a string
    assert not isinstance(hailo.HAILO_LANDMARKS, str), "HAILO_LANDMARKS should be an enum, not a string"
    ...
    for detection in detections:
        if detection.get_label() == "person" and detection.get_confidence() >= confidence_threshold:
            ...
            if landmarks:
                ...
                left_x, left_y = int(left_wrist.x() * SCREEN_WIDTH), int(left_wrist.y() * SCREEN_HEIGHT)
                right_x, right_y = int(right_wrist.x() * SCREEN_WIDTH), int(right_wrist.y() * SCREEN_HEIGHT)
