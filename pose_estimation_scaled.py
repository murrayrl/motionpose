    ...
    for idx, det in enumerate(detections):
        ...
        for kp_idx, kp_name in KEYPOINTS.items():
            if len(pts) > kp_idx:
                ...
                # Add to OSC data if in OSC keypoint list (scaled)
                if kp_name in OSC_KEYPOINTS:
                    list_x[f"{kp_name}/p{idx + 1}"] = float(x_scaled)
                    list_y[f"{kp_name}/p{idx + 1}"] = float(y_scaled)
                
                # Print for debugging
                print(f"Person {idx + 1}: {kp_name} at ({x_scaled:.1f}, {y_scaled:.1f})")
...
def send_osc(x_list, y_list):
    ...
    for (key1, value1), (key2, value2) in zip(x_list.items(), y_list.items()):
        ...
        try:
            ...
            print(f"OSC: {channel_x} = {value1:.1f}, {channel_y} = {value2:.1f}")
        except Exception as e:
